        ...
        if game_step > self.arglist.learning_start_step and \
            ...
            for i in range(self.batch):
                ...
                for agent_idx in range(self.n):
                    ...
                    grad_fn1_ac = ms.ops.value_and_grad(forward_fn1, None, self.optimizers_ac[agent_idx].parameters,
                                                       has_aux=True)
                    grad_fn1_rnn = ms.ops.value_and_grad(forward_fn1, None, self.optimizers_rnn[agent_idx].parameters,
                                                       has_aux=True)
                    _, grads1 = grad_fn1_ac()
                    self.optimizers_ac[agent_idx](grads1)
                    _, grads2 = grad_fn1_rnn()
                    self.optimizers_rnn[agent_idx](grads2)
                ...
                for agent_idx in range(self.n):
                    ...
                    def forward_fn2():
                        ...
                        return loss_c, q
                    grad_fn2 = ms.ops.value_and_grad(forward_fn2, None, self.optimizers_cr[agent_idx].parameters,
                                                       has_aux=True)
                    _, grads = grad_fn2()
                    self.optimizers_cr[agent_idx](grads)
    ...
    def train(self, game_step):
        ...
        if game_step > self.arglist.learning_start_step and \
            ...
            for agent_idx in range(self.n):
                ...
                grad_fn1_c = ms.ops.value_and_grad(forward_fn1, None, self.optimizers_cr[agent_idx].parameters, has_aux=True)
                grad_fn1_a = ms.ops.value_and_grad(forward_fn1, None, self.optimizers_at[agent_idx].parameters, has_aux=True)

                _, grads1 = grad_fn1_c()
                self.optimizers_cr[agent_idx](grads1)
                _, grads2 = grad_fn1_a()
                self.optimizers_at[agent_idx](grads2)
                ...
                def forward_fn2():
                    ...
                    loss = (1e-3 * loss_pse + loss_a)
                    return loss, model_out

                grad_fn2_ac = ms.ops.value_and_grad(forward_fn2, None, self.optimizers_ac[agent_idx].parameters, has_aux=True)
                grad_fn2_at = ms.ops.value_and_grad(forward_fn2, None, self.optimizers_at[agent_idx].parameters, has_aux=True)

                _, grads1 = grad_fn2_ac()
                self.optimizers_ac[agent_idx](grads1)
                _, grads2 = grad_fn2_at()
                self.optimizers_at[agent_idx](grads2)
